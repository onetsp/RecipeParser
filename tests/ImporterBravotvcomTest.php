...
class ImporterBravotvcomTest extends PHPUnit_Framework_TestCase {

    public function test_cleanup_times() {
        $time = Importer_Bravotvcom::cleanupTime(' 2 Hours ');
        $this->assertEquals('2 hours', $time);

        $time = Importer_Bravotvcom::cleanupTime(' Under 2 Hours ');
        $this->assertEquals('2 hours', $time);

        $time = Importer_Bravotvcom::cleanupTime(' Two  Hours ');
        $this->assertEquals('2 hours', $time);
    }
